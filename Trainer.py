...
# Router for the multipage app. Pages live in views/ (not a pages/ folder) so Streamlit
# doesn't auto-list them, and we only add the admin page for admins. The admin page still
# guards itself, so hiding the link is cosmetic rather than a real security boundary.
...
# Optional login lives at the bottom of the sidebar, under the nav. It is purely additive,
# as the trainer plays anonymously without it. Logging in just saves progress and unlocks
